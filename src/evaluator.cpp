...
steps_t Evaluator::eval( const Program &p, Sequence &seq, int64_t num_terms, bool throw_on_error )
{
  if ( num_terms < 0 )
  {
...
steps_t Evaluator::eval( const Program &p, std::vector<Sequence> &seqs, int64_t num_terms )
{
  if ( num_terms < 0 )
  {
    num_terms = settings.num_terms;
...
std::pair<status_t, steps_t> Evaluator::check( const Program &p, const Sequence &expected_seq,
